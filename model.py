...
class DecisionTreeClassifierAltered(DecisionTreeClassifier):
    ...
    def _best_split(self, X, y):
        ...
        if m <= 1:
            return None, None
        num_parent = np.array([np.sum(y == c) for c in range(self.n_classes_)])
        majority_class = num_parent.argmax()
        best_acc = accuracy_score(y, [majority_class for _ in range(len(y))])
        best_idx, best_thr = None, None
        for idx in range(self.n_features_):
            thresholds, classes = zip(*sorted(zip(X[:, idx], y)))
            num_left = np.array([0] * self.n_classes_)
            num_right = num_parent.copy()
            for i in range(1, m):
                c = classes[i - 1]
                num_left[c] += 1
